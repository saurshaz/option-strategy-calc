...
MessageHandler.prototype.onMessageHandler = function(request, sender, sendResponse) {
  ...
  if (request.action == 'getCookies') {
    var url = request.url;
    var urlOk = false;
    for (let i = 0; i < this.allowedRemoteUrls.length; ++i) {
    ...
    window.chrome.cookies.get({url: url, name: request.cookieName},
...
MessageHandler.prototype.install = function() {
  window.chrome.runtime.onMessage.addListener(this.onMessageHandler.bind(this));
};
